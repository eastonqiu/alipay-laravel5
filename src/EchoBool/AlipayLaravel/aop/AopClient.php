...
class AopClient {
	...
	protected function smt_curl($url, $postFields = null) {
		$ch = curl_init();
		curl_setopt($ch, CURLOPT_URL, $url);
		curl_setopt($ch, CURLOPT_FAILONERROR, false);
		curl_setopt($ch, CURLOPT_RETURNTRANSFER, true);
		...
		curl_setopt($ch, CURLOPT_HTTPHEADER, $headers);
        curl_setopt($ch, CURLOPT_CUSTOMREQUEST, 'POST');//获取跳转后的址址
		$reponse = curl_exec($ch);
	...
	*/
	...
	public function smtExecute($request, $authToken = null, $appInfoAuthtoken = null) {
		...
		//发起HTTP请求
		try {
			$resp = $this->smt_curl($requestUrl, $apiParams);
		} catch (\Exception $e) {
			$this->logCommunicationError($sysParams["method"], $requestUrl, "HTTP_ERROR_" . $e->getCode(), $e->getMessage());
			return false;
		}
